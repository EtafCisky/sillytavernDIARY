...
async function encryptAuthorName(authorName) {
  const base64Encoded = encodeBase64(authorName);
  const sha256Hashed = await sha256Hash(base64Encoded);
  return sha256Hashed;
}
...
async function verifyAuthorInfo() {
  ...
  try {
    ...
    const manifestData = await $.getJSON(manifestPath);
    const manifestAuthor = manifestData.author || '';

    const encryptedManifestAuthor = await encryptAuthorName(manifestAuthor);

    const isValid = encryptedManifestAuthor === AUTHOR_VERIFICATION_PASSWORD;
